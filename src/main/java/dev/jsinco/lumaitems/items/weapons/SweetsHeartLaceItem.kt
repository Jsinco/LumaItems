...
class SweetsHeartLaceItem : CustomItem {
    ...
    override fun createItem(): Pair<String, ItemStack> {
        ...
        return Pair("sweetsheartlace", item.createItem())
    }

    override fun executeAbilities(type: Ability, player: Player, event: Any): Boolean {
        when (type) {
            Ability.PROJECTILE_LAND -> {
                if (Random.nextInt(100) > 25) return false
                event as ProjectileHitEvent
                val entity = event.hitEntity ?: return false


                entity.world.spawnParticle(Particle.HEART, entity.location, 10, 0.5, 0.5, 0.5, 0.5)

                if (entity is Enemy) {
                    val nameSpace = NamespacedKey(p, "sweetsheartlace")
